...
from OneNoise.network.blocks import NeRFPosEnc, Block as SpadeBlock, ResnetBlock as SpadeResnetBlock
...
def Normalize(in_channels, cond_emb = None, num_groups=32):
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, kernel_size=3, conv_shortcut=False,
                 dropout, temb_channels=512, normalize=True, spade_cond_channels = None):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 attn_type="vanilla", use_spade = False, pos_enc_deg=0, nparams=18, nclasses=1, cond_dim=128, **ignorekwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
        self.temb_ch = 0
        ...
        if self.use_spade:
            print(f"[DEBUG SPADE] Decoder initialized with SPADE. cond_dim={cond_dim}, nclasses={nclasses}, nparams={nparams}")
        self.cls_dim = ch if nclasses > 1 else 0
        ...
        if self.use_spade:
            self.spade_cond_mlp = nn.Sequential(
    ...
    def preproc_conditioning(self, params, classes, class_emb=None):
        '''
        Preprocesses conditioning features.
        ...
        if class_emb is not None:
            ...
            cls_embs = torch.einsum('b c h w, c d -> b d h w', classes, self.classes_emb.weight)
        c = torch.cat((params, cls_embs), dim = 1) # (B, C' + cls_emb, H, W)
        c = torch.permute(c, (0, 2, 3, 1)) # (B, H, W, C' + cls_emb)
        c = self.spade_cond_mlp(c)
    ...
    def forward(self, z, classes = None, substance_params = None):
        ...
        c = None
        if self.use_spade and substance_params is not None and classes is not None:
            c = self.preproc_conditioning(substance_params, classes)
        ...
        # middle
        h = self.mid.block_1(h, temb, c)
        h = self.mid.attn_1(h)
        h = self.mid.block_2(h, temb, c)

        # upsampling
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                h = self.up[i_level].block[i_block](h, temb, c)
                if len(self.up[i_level].attn) > 0:
                    h = self.up[i_level].attn[i_block](h)
            if i_level != 0:
                h = self.up[i_level].upsample(h)
